 ...
 * theme-switcher.js
 *
 * Style switcher: instant CSS swap within the current theme's styles.
 * Theme switching is not supported at runtime (themes are build-time in Astro).
 *
 * Activated when BasePage.astro injects window.__DEMO_CFG__ (demo_mode: true).
 *
 * localStorage keys:
 *   style_override  — e.g. "style-1-editorial"
 */
(function () {
  const cfg = window.__DEMO_CFG__;
